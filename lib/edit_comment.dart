...
import 'package:social_media_app/search_tag_users.dart';
import 'package:social_media_app/appdata/global_library.dart';
import 'package:social_media_app/class/media_data_class.dart';
import 'package:social_media_app/class/comment_class.dart';
import 'package:social_media_app/custom/custom_button.dart';
import 'package:social_media_app/custom/custom_text_editing_controller.dart';
...
class __EditCommentWidgetStatefulState extends State<_EditCommentWidgetStateful> with LifecycleListenerMixin{
  CustomTextFieldEditingController commentController = CustomTextFieldEditingController();
  ...
  ValueNotifier<bool> verifyCommentFormat = ValueNotifier(false);
  ...
  void initState(){
    ...
    commentController.addListener(() {
      if(mounted){
        ...
        verifyCommentFormat.value = commentText.isNotEmpty && commentText.length <= maxPostWordLimit;
      }
    });
  }
  ...
  void initializeEditedPost() async{
    if(mounted){
      ...
      CommentClass commentData = widget.commentData;
      commentController.text = commentData.content;
      for(int i = 0; i < commentData.mediasDatas.length; i++){
        MediaDatasClass e = commentData.mediasDatas[i];
  ...
  void editComment() async{
    try {
      if(mounted){
        if(!isLoading.value && (commentController.text.isNotEmpty || mediasDatas.value.isNotEmpty)){
          isLoading.value = true;
          String commentID = widget.commentData.commentID;
          CommentClass previousCommentData = appStateClass.commentsNotifiers.value[widget.commentData.sender]![commentID]!.notifier.value;
          ...
          List<String> hashtags = (textDisplayHashtagRegex.allMatches(commentController.text).map((match) => match.group(0)).map((str) => str!.substring(1).toLowerCase()).toSet().toList());
          ...
          if(res.data.isNotEmpty){
            if(res.data['message'] == 'Successfully edited the comment'){
              CommentClass commentDataClass = CommentClass(
                commentID, 'comment', commentController.text, appStateClass.currentID, previousCommentData.uploadTime,
  ...
  void listenTextField(String value){
    if(isUserTagged(commentController)){
  ...
  void userIsTagged(List<String> usernames, List<String> userIDs){
    for(int i = 0; i < usernames.length; i++){
      ...
      final int cursorPosition = commentController.selection.start;
          ...
          ValueListenableBuilder<bool>(
            ...
            builder: (context, bool isLoadingValue, child){
              return ValueListenableBuilder<bool>(
                valueListenable: verifyCommentFormat,
                builder: (context, bool commentVerified, child){
                    ...
                    builder: (context, List<MediaDatasClass> mediasDatasValue, child){
                      return Padding(
                        ...
                        child: CustomButton(
                          ...
                          onTapped: !isLoadingValue && (mediasDatasValue.isNotEmpty || commentVerified) ? () => editComment() : null,
